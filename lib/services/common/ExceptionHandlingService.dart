...
class ExceptionHandlingService implements IExceptionHandlingService {
  ...
  Future<void> handleException(dynamic exception) async {
    ...
    await FirebaseCrashlytics.instance.log('${StackTrace.current.toString()} / ${!(exception is CustomException) ? exception : (exception).message}');
    FirebaseCrashlytics.instance.setCustomKey('ExceptionType', "${!(exception is CustomException) ? 'Exception' : 'CustomException'}");
